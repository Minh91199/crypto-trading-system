...
public class TradeServiceImpl implements TradeService {
  ...
  public TradeResponse executeTrade(TradeRequest tradeRequest) {
    ...
    updateWalletBalances(
        usdtWallet, coinWallet, orderType, tradeRequest.getQuantity(), tradeAmount, coinCurrency);

    // Create and save trade transaction
    TradeTransaction savedTransaction =
        createAndSaveTransaction(
            user, symbol, orderType, executedPrice, tradeRequest.getQuantity());
  ...
  public List<TradeHistoryResponse> getTradeHistory(Long userId) {
    ...
    List<TradeTransaction> transactions =
        tradeTransactionRepository.findByUserOrderByTradeTimeDesc(user);
  ...
  private void validateSymbol(String symbol) {
    if (!supportedSymbols.contains(symbol)) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Symbol " + symbol + " is not supported");
    }
  }
  ...
  private Wallet getUsdtWallet(CryptoUser user) {
    return walletRepository
        .findByUserAndCurrency(user, CurrencyConstant.STABLE_COIN)
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, CurrencyConstant.STABLE_COIN + " wallet not found"));
  }
  ...
  private Wallet getOrCreateCoinWallet(CryptoUser user, String coinCurrency) {
    return walletRepository
        .findByUserAndCurrency(user, coinCurrency)
        .orElseGet(
            () ->
  ...
  private Wallet getCoinWallet(CryptoUser user, String coinCurrency) {
    return walletRepository
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Insufficient " + coinCurrency + " funds"));
  }

  private AggregatedPrice getLatestAggregatedPrice(String symbol) {
    return aggregatedPriceRepository
        .findTopBySymbolOrderByCreatedAtDesc(symbol)
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "No aggregated price available for symbol " + symbol));
  }
  ...
  private void updateWalletBalances(
    ...
    if (orderType == TradeType.BUY) {
      if (usdtWallet.getBalance().compareTo(tradeAmount) < 0) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "Insufficient " + CurrencyConstant.STABLE_COIN + " funds");
      }
  ...
  private TradeTransaction createAndSaveTransaction(
      CryptoUser user,
      String symbol,
      TradeType orderType,
      BigDecimal executedPrice,
  ...
  private TradeType parseOrderType(String orderTypeStr) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Invalid order type: " + orderTypeStr);
    }
  }
  ...
  private void validateQuantity(String symbol, BigDecimal quantity) {
    ...
    if ("BTC".equals(coin) && quantity.compareTo(minBtcQty) < 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Minimum trade quantity for BTC is " + minBtcQty);
    }
    if ("ETH".equals(coin) && quantity.compareTo(minEthQty) < 0) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Minimum trade quantity for ETH is " + minEthQty);
    }
  }
}
